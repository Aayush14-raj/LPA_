...
document.getElementById("createLpaBtn").addEventListener("click", () => {
  ...
  document.getElementById("downloadPdfBtn").addEventListener("click", () => {
    ...
    window.open(`http://localhost:5000/api/download-lpa-excel/${plant}/${month}/${year}`, "_blank");
  });
...
if (!uploadedData || 
  ...
  return alert("⚠️ Please upload the filled Excel template first.\nVSL, CFT, CQE & PH data are required.");
    ...
    // 🌿 NEW — FALLBACK VALUE STREAM LOGIC (use built-in config if Excel didn't include streams)
    if (!uploadedData.valueStreams || !Object.keys(uploadedData.valueStreams).length) {
      if (PLANT_CONFIG[plant]) {
        const cfg = PLANT_CONFIG[plant];
        const vsCounts = {};
        Object.entries(cfg).forEach(([vs, arr]) => {
          vsCounts[vs] = Array.isArray(arr) ? arr.length : Number(arr) || 0;
        });
        uploadedData.valueStreams = vsCounts;
        uploadedData.sublineNames = cfg;
        localStorage.setItem("uploadedExcelData", JSON.stringify(uploadedData));
      }
    }

    // ✅ Ensure Value Stream Configuration exists
    ...
    if (!data || !data.assignments || !data.assignments.length) {
      console.error("❌ No assignments generated by createLpaCalendarData");
      return alert("⚠️ No assignments were generated. Please check your Excel file or logic.");
    }

    // ✅ Attach uploaded data (needed for backend mailing)
    ...
    // ✅ Get month & year for backend
    const now = new Date();
    const month = now.toLocaleString("default", { month: "long" });
    const year = now.getFullYear();

    // ============================================================
    // 🔹 Step 1: Save generated calendar (optional)
    // ============================================================
    const saveResponse = await fetch(`${API_BASE}/api/lpa-calendar`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    if (!saveResponse.ok) {
      console.warn("⚠️ Warning: Calendar not saved to backend (non-critical)");
    }

    // ============================================================
    // 🔹 Step 2: Request backend to generate Excel & send email
    // ============================================================
    ...
    if (mailResult.success) {
      ...
      console.error("❌ Email sending failed:", mailResult);
      alert("⚠️ Calendar saved but failed to send mail. Check backend logs.");
    }

  } catch (err) {
    console.error("❌ Error generating or mailing LPA Calendar:", err);
